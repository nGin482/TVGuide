...
const useShow = () => {
  ...
  const updateEpisodeContext = (
    showName: string,
    episodeId: number,
    updatedEpisode: ShowEpisode
  ) => {
  ...
  const deleteEpisodeFromContext = (showName: string, episodeId: number) => {
    const show = shows.find(show => show.show_name === showName);
    const updatedEpisodes = show.show_episodes.filter(
