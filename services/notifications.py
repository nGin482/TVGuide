...
from exceptions.DatabaseError import DatabaseError, SearchItemAlreadyExistsError, SearchItemNotFoundError, SeasonNotFoundError, ShowNotFoundError
...
async def season_details(ctx: Context, show: str, season: str):
    ...
    except (ShowNotFoundError, SeasonNotFoundError) as err:
        await ctx.send(f'Error: {str(err)}')